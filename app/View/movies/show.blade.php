...
<div class="container mx-auto px-4">
    <div class="flex flex-col md:flex-row">
        ...
        <div class="md:w-2/3 md:pl-8 mt-4 md:mt-0">
            ...
            <p class="text-gray-600 mb-4">{{ $movie['release_date'] }} | {{ implode(', ', array_column($movie['genres'], 'name')) }}</p>
            <p class="mb-4">{{ $movie['overview'] }}</p>
            <div class="mb-4">
                <h2 class="text-xl font-semibold mb-2">Casting</h2>
                <p>{{ implode(', ', array_slice(array_column($movie['credits']['cast'], 'name'), 0, 5)) }}</p>
            </div>
            <div>
                <h2 class="text-xl font-semibold mb-2">Réalisateur</h2>
                <p>{{ collect($movie['credits']['crew'])->firstWhere('job', 'Director')['name'] ?? 'Non spécifié' }}</p>
            </div>
        </div>
    </div>
    ...
    <section class="mt-8">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
            @foreach($similarMovies as $similarMovie)
                <x-media-card :media="$similarMovie" type="movie" />
            @endforeach
        </div>
    </section>
